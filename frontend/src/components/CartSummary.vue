...
<template>
  <div class="bg-gray-100 p-4 rounded">
    ...
    <ul v-if="orderStore.cart.length > 0">
      <li v-for="item in orderStore.cart" :key="item.id" class="flex justify-between items-center border-b py-2">
        <span class="flex-grow">{{ item.name }}</span>
        <input
          type="number"
          class="w-16 p-1 text-center border rounded"
          :value="item.quantity"
          @input="validateQuantity(item.id, $event)"
          min="1"
        />
        <button @click="orderStore.removeFromCart(item.id)" class="ml-4 text-red-600">Remove</button>
      </li>
    </ul>
